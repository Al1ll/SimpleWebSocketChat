...
type ChatMessengeHandler()=
  ...
  member this.SendMessageToAll (msg:string) = async {
    ...
    |> Seq.iter(fun ws -> 
      match ws.State with
      | WebSocketState.Open -> 
          ws.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes(msg),0,msg.Length),WebSocketMessageType.Text,true,CancellationToken.None)
          |>Async.AwaitTask
  ...
  member this.OnDisconnet nickname = async {
    try
      let ws = sockets.TryRemove nickname |> snd
